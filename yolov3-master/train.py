...
hyp = {'giou': 10,      # giou loss gain
       'xy': 4.062,     # xy loss gain
       'wh': 4.1845,    # wh loss gain
       'cls': 10,       # cls loss gain
       'cls_pw': 2,     # cls BCELoss positive_weight
       'obj': 10,       # obj loss gain
       'obj_pw': 50,    # obj BCELoss positive_weight
...
def train(
    ...
    if opt.resume or opt.transfer:  # Load previously saved model
    ...
    elif restart == True:  # Initialize model with backbone (optional)
        if '-tiny.cfg' in cfg:
        ...
        print("加载权重。。。")
        chkpt = torch.load('weights/best_1009_tiny.pt', map_location=device)  # load checkpoint 
        model.load_state_dict(chkpt['model'])
        cutoff = 15

    # 学习率调整 
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[100,1000,3000], gamma=0.1)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Freeze backbone at epoch 0, unfreeze at epoch 1 (optional) --------------------------- 冻结参数
        ...
        if not (opt.notest or (opt.nosave and epoch < 10)) or epoch == epochs - 1:
            with torch.no_grad():
                results, maps = test.test(cfg, data_cfg, batch_size=16, img_size=opt.img_size, model=model,
                                          conf_thres=0.1)
