...
def compute_loss(p, targets, model, giou_loss=True):  # predictions, targets, model
    ...
    # BCEcls = nn.CrossEntropyLoss(weight=ft(h['class_weights']))
    # CEcls = nn.CrossEntropyLoss()  # (weight=model.class_weights)
    ...
    for i, pi0 in enumerate(p):    # layer i predictions, i
        ...
        if nb:  # number of targets
            ...
            try:
                ...
                print("a:{},b{},gj{},gi{}".format(b, a, gj, gi))
                print("pi0.size:{}".format(pi0.size()))
            ...
            #pcls_ = F.softmax(pi[..., 5:], dim=1)
            #lcls += CEcls(pcls_, tcls[i])  # cls loss (CE)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    tl = line_thickness or round(0.0005 * max(img.shape[0:2])) + 1  # line thickness
